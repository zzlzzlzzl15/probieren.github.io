...
# ╔═╡ dcd4c74b-0b59-4d1b-ba98-038c642d1793
md"""
...
md"""
If there are two unique signals A and B, a new signal is generated by mixing with a specific rate. The mixed signals are what we usually measure. In order to unmix the original signal, we can invert the mixing matrix $A$ to $A^{-1}$ and undo the mixing operations.
"""
...
md"""
### Step2. Whitening
As for two signals, the multi information is minimal after rotation\
Every mixing has stretching operations such as $2 *A$ and rotation operations in the signal. Therefore we have to undo the stretching operations and rotation operations. In the first step, the "whitening" operation or normalization is executed, after whitening the variables are uncorrelated(does not imply independence), and also the mixing matrix is now orthogonal.
...
# ╔═╡ 6e811f06-51f3-493e-a376-ed71bef64291
md"""
## ICA Algothem
"""md
