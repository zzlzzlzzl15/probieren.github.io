...
# ╔═╡ f61d554d-2e2f-4671-bf4e-336f2d05cb13
using CairoMakie
...
begin
	...
	sidebar = Div([
		md"""##### Change it up""",
		md"""Select signal original A$(test = @bind signalA Select(["sin.(xs)","cos.(xs)"],default="sin.(xs)"))""",
		md"---",
		md"""Select signal original B$(test = @bind signalB Select(["sin.(3xs)","cos.(3xs)"],default="cos.(3xs)"))""",
		md"""How many samples? $(@bind point_number PlutoUI.Slider(100:10:1000;default=500,show_value=true))""",
		md"---",
		md"""Select mixing parameter a$(test = @bind a Select(["1","2","3"],default="1"))""",
		md"""Select mixing parameter b$(test = @bind b Select(["-1","-2","-3"],default="-2"))""",
		md"""Select mixing parameter c$(test = @bind c Select(["1.5","1.73","3"],default="1.73"))""",
		md"""Select mixing parameter d$(test = @bind d Select(["1.5","1.73","3.41"],default="3.41"))""",
		md"---",
...
# ╔═╡ 6891a6cb-76cb-4d67-b056-342367e356c5
md"""
If there are two unique signals A and B, and a new signal is generated by mixing with a certain rate. THe mixed signals are what we usually measures. In order to unmix the orighinal signal, we can invert the mixing matrix $A$ to $A^{-1}$ and undo the mixing operations.
"""


# ╔═╡ 9ad0b589-2020-4a87-845d-33c665dde10f
md"""
## Mathmatic varibles and operations
*n*: number of samples\
*i*: number of sources\
...
*$A$*: mixing matrix [i∗j] \
*$A^{-1}$*: unmixing matrix [i∗j]\
The ICA model:\
$\qquad \qquad \qquad s=Ax$\
we are looking forward to finding teh mixing matrix *$A$*, which could minimizes the dependence of each elements varaibles in side S(the original underlying sources),
Thus, we show that *$A^{-1}$* is equal to the unmixing matrix\
$\qquad \qquad \qquad s=Ax$\
$\qquad \qquad \qquad A^{-1}x=A^{-1}As$\
$\qquad \qquad \qquad A^{-1}x=s$\

"""

# ╔═╡ b9a624e1-b6d7-4dd1-b4de-27cc0cd81fe2
begin
xs = range(0, 10, length=point_number)
if signalA=="sin.(xs)"
	A = sin.(xs)
elseif signalA=="cos.(xs)"
	A = cos.(xs)
end
if signalB=="sin.(3xs)"
	B = sin.(3xs)
elseif signalB=="cos.(3xs)"
	B = cos.(3xs)
end
end

# ╔═╡ 1feda2cd-158c-4a3e-84af-7427e82921b6
begin
	md"""
 #### Figure 1. Two Original Signal
"""
	f = Figure()
	g1 = f[1, 1]
	g11 = f[2, 1]
	scatter(g1, xs, A, color = :red, markersize = 5)
	scatter!(g1, xs, B, color = :blue, markersize = 10)
	scatter(g11,A,B, color = :red, label = "Red Dots")
	f
end

# ╔═╡ a95de827-d0ed-4a59-96a3-17eed040e3c4
md"""
### Step1. Demeaning
In the first step we have to demeaning the signal, that means we need to shift the base line to be 0 for each collected signals\
Demeaning(also called centering):\
$\qquad \qquad \qquad x_{m}=x-E[x]$\
""" 
...
"""
	line1 = parse(Float64, a)*A + parse(Float64, b)*B
	line2 =   parse(Float64, c)*A + parse(Float64, d)*B
	f2 = Figure()
	g2 = f2[1, 1] 
	g22 = f2[2, 1]
	scatter(g2, xs, line1, color = :red, markersize = 5)
	scatter!(g2, xs, line2, color = :blue, markersize = 10)
	scatter(g22,line1, line2, color = :red, label = "Red Dots")
	f2
end
...
"""
	...
	g3 = f3[1, 1] 
	g33 = f3[2, 1]
	scatter(g3,xs, whitening_matrix[1,:], color = :red, markersize = 5)
	scatter!(g3,xs, whitening_matrix[2,:], color = :blue, markersize = 10)
	...
	scatter(g33,x1, x2, color = :red, label = "Red Dots")
	f3
end
...
begin
	...
	g4 = f4[1, 1]
	g44 = f4[2, 1]
	scatter(g4,xs, unmixed_line1, color = :red, markersize = 5)
	scatter!(g4,xs, unmixed_line2, color = :blue, markersize = 10)
	scatter(g44, unmixed_line1,unmixed_line2, color = :red,label = "Red Dots")
	f4
end
