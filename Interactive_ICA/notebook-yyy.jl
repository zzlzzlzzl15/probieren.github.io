...
# ╔═╡ 5de2f965-39fa-4931-baeb-3d6bc2edb44a
begin
...
# ╔═╡ 51b13bd0-d2c6-11ec-3d64-6119eddff390
md"""
...
# ╔═╡ a37868aa-82a4-4220-8ee3-d4bb44b34973
html"""
 <style>
...
# ╔═╡ 3115fabe-33cc-4557-bcb8-f509f99afa2e
begin
	sidebar = Div([
		md"""##### Change it up""",
		md"""Select signal original A$(test = @bind signalA Select(["sin.(T1xs)","cos.(T1xs)"],default="sin.(T1xs)")) """,
		md"---",
		md"""Choose the value for T1 $(@bind T1 PlutoUI.Slider(1:1:10;default=1,show_value=true))""",
		md"---",
		md"""Select signal original B$(test = @bind signalB Select(["sin.(T2xs)","cos.(T2xs)"],default="cos.(T2xs)"))""",
		md"---",
		md"""Choose the value for T2 $(@bind T2 PlutoUI.Slider(1:1:10;default=3,show_value=true))""",
		md"""How many samples? $(@bind point_number PlutoUI.Slider(100:10:1000;default=500,show_value=true))""",
		md"---",
...
# ╔═╡ b9a624e1-b6d7-4dd1-b4de-27cc0cd81fe2
begin
...
# ╔═╡ f8b6be4e-a09d-4e90-9782-0d3f71f2d4bd
if signalA=="sin.(T1xs)"&&signalB=="sin.(T2xs)"&&T1==T2||signalA=="cos.(T1xs)"&&signalB=="cos.(T1xs)"&&T1==T2
	print("ICA can't function well, because A and B are identical function")
end

# ╔═╡ aae4f828-b678-43dc-b140-bc9916584151
B

# ╔═╡ a92ddfc6-79d3-403d-a461-5c9b16da32e7
...
function original_signal()
	...
	g1b = f[2,2]
	g1 = f[1, 2]
	scatter(g1, xs, A, color = :skyblue, markersize = 5,label = "$A_label( $T1 xs）",axis=(; xlabel=L"x", ylabel="f(x)"))
	scatter!(g1, xs, B,  markersize = 10, label = "$B_label( $T2 xs）")
	axislegend()
	density(g1a,A,color = :skyblue)
	density(g1b,B,direction = :y)
	g11 = f[2, 1]
	postion = scatter(g11,A,B, color = :salmon, label = "position", axis=(; xlabel=L"f1(x)", ylabel="f2(x)"))
	axislegend()
	f
...
# ╔═╡ 51282a4c-645b-4cee-abc7-5602226d9081
original_signal()

# ╔═╡ 08f0b034-fe85-451c-b70d-f99aa0c09cb3
begin
	
	linex = a*A + b*B
end

# ╔═╡ c4309d91-6daf-4cb1-816a-2e5f01fe1a46
begin
	md"""
...
"""
	...
	g2a = f2[1,1]
	g2b = f2[2,2]
	scatter(g2, xs, line1, color = :skyblue, markersize = 5, axis=(; xlabel=L"x", ylabel="f(x)"),label = "$a$A_label( $T1 xs）+ $b$B_label( $T2 xs）")
	scatter!(g2, xs, line2, markersize = 10, label = "$c$A_label( $T1 xs）+ $d$B_label( $T2 xs）")
	axislegend()
	density(g2a,line1,color = :skyblue)
	density(g2b,line2,direction = :y)
	g22 = f2[2, 1]
	scatter(g22,line1, line2, color = :salmon, axis=(; xlabel=L"f1(x)", ylabel="f2(x)"),label = "Position")
	axislegend()
	f2
end

# ╔═╡ 61569689-4a20-4dd3-8ffb-c38b7dad1937
begin
	md"""
calculate the demeaning data
"""
using StatsBase
using MultivariateStats
	...
	line1_demean = line1 - fill(mean1, (point_number,1))
	line2_demean = line2 - fill(mean2, (point_number,1))#change
	
end

# ╔═╡ 652c1270-dbf0-42ef-8e94-60b6a72bc5d4
begin
...
# ╔═╡ ecbce097-cef5-4df3-a898-b196423e8740
md"""
### Step2. Whitening
As for two signals, the multul infomation is minimal after rotation\
For every mixing has stretching operation such as $2 *A$ and rotation operations in the signal. Therefore we have to undo the streaching operations and rotation operations. In the first step, the "whitening" operation or normalization is executed, after whitening the varaibles are uncorrelated(does not imply independence) and also the mixing matrix is now orthogonal.
$\qquad \qquad \qquad Cov(N)=\frac{1}{n-1}\left({(N*N')}\right)$\
$\qquad \qquad \qquad \qquad \, \, \, \,C=Cov(x_m)$\
$\qquad \qquad \qquad \qquad \, \, \, \, V=C^{-\frac{1}{2}}$\
$\qquad \qquad \qquad \, Cov(P)=I=Cov(V*x_m)$\
$\qquad \qquad \qquad \qquad \qquad \, \, \, \, \, \, \, \, =Cov(C^{-\frac{1}{2}}*x_m)$\
$\qquad \qquad \qquad \qquad \qquad \, \, \, \, \, \, \, \, =(C^{-\frac{1}{2}}*x_m)(C^{-\frac{1}{2}}*x_m)'$\
$\qquad \qquad \qquad \qquad \qquad \, \, \, \, \, \, \, \, =C^{-\frac{1}{2}}*x_m*x_m*C^{-\frac{1}{2}}$\
$\qquad \qquad \qquad \qquad \qquad \, \, \, \, \, \, \, \, =C^{-\frac{1}{2}}*C*C^{-\frac{1}{2}}=I$\
Then multiplying $Vx_{m}=C^{-\frac{1}{2}}*x_{m}$ returns the *x_{w}*, the whitened signal
"""

# ╔═╡ c22d544c-4bb1-48d3-bd72-55e2aecd94f2
begin
	whitening_matrix = transpose(M.W) * demeaning_matrix
end

# ╔═╡ c6a424fa-b457-4ec6-9bfb-9f7e89bb3386
begin
	md"""
#### Figure 3 Whiten Mixed signal
"""
	f3 = Figure()
	g3 = f3[1,2] 
	g3a = f3[1,1]
	...
	density(g3b, whitening_matrix[2,:],direction = :y)
	scatter(g3,xs, whitening_matrix[1,:], color = :skyblue, markersize = 5, axis=(; xlabel=L"x", ylabel="f(x)"), label = "whiten Mixed signal1")
	scatter!(g3,xs, whitening_matrix[2,:], markersize = 10, label = "whiten Mixed signal2")
	axislegend()
	...
	g33 = f3[2, 1]
	scatter(g33,x1, x2, color = :salmon, label = "Position", axis=(; xlabel=L"f2(x)", ylabel="f2(x)"))
	axislegend()
	f3
end

# ╔═╡ 60c4829e-43d8-4618-8671-918b413a5c44
md"""
### Step3. Fast ICA
...
#### Maximizing statistical independence(As for two signals)
...
PS: Mutual information:
The mututal infomation is the uncertainty of two varaibles
$\qquad \qquad \qquad I(X;Y)=\int_X\int_Y{P(X,Y)log\frac{P(X,Y)}{P(X)P(Y)}}$\
If the mututal infomation is equal to zero $I(X;Y)=0$, the varaible X and Y are independent.\
\

#### Minimizing normality(As for multiple signals)
...
#### Now we would like to discuss the FastICA algorithm(minimizing normality)
...
We try minimize negentropy, using this formular\
\
**1. $wi$ is the i-th column of the unmixing matrix - we estimate each component iteratively.**\
**2. We try minimize negentropy, using this formular**\
**3. The weightvector is normalized (c.f. Limitations)**\
**4. If we want to estimate the 2nd, 3rd ... source, we need to make sure the unmixing weights are orthogonal**\
**5. Make the weight-column vector orthogonal to the other vectors (why does this work?)**\
**6. Normalize again**\
**7. Start from the beginning, or finishs**\


"""

# ╔═╡ 12e27174-45a2-4bce-82dd-1db3e44260a9
md"""
The correlated steps:\
Negetropy:\
$\qquad \qquad \qquad \, H(X)=H(X_{Gauss})-H(X)$\
$\qquad \qquad \qquad \, \phi(w_i^TX)=tanh(w_i^TX)$\
calculate until the $X_{Gauss}$ has the same covariance matrix as X\
**1. Initialize $wi$ randomly**\
**2. $w_i^+=E(X\phi(w_i^TX))-E(\phi'(w_i^TX))w_i$**\
**3. $w_i=\frac{w_i^+}{||w_i^+||}$**\
...
#### Now we would like to discuss method about infomax algorithm
...
The standard infomax algorithm with $f(Y)=Tanh(Y)$ is only suitable for super-gaussian distributions, as in sub-gaussian distributions the function gets negative. Therfore an extension to the infomax algorithm "extended infomax" is used, where the function used is
$\qquad \qquad \qquad \, w^+=w-[E{x\phi(w_Tx)}-\beta w]/[E{g'(w^Tx)}-\beta]$\
...
# ╔═╡ 64bc63e3-51bd-42ee-bf85-417f9605d39f
md"""
...
# ╔═╡ f132684f-82ed-4694-99f3-0ddeb5476943
begin
	md"""
#### Rotation(FastICA)
"""
...
# ╔═╡ 31580f6d-902f-4aea-bda9-075c07169288
md"""
#### Figure 4. Unmixed Signal
"""

# ╔═╡ 97e01b3f-9d60-4abe-8c66-80bfe9e244db
begin
	...
	density(g4b, unmixed_line2,direction = :y)
	scatter(g4,xs, unmixed_line1, color = :skyblue, markersize = 5, axis=(; xlabel=L"x", ylabel="f(x)"), label = "Reduced signal1")
	scatter!(g4,xs, unmixed_line2, markersize = 10, label = "Reduced signal2")
	axislegend()
	
	g44 = f4[2, 1]
	scatter(g44, unmixed_line1,unmixed_line2, color = :salmon,label = "Position", axis=(; xlabel=L"f1(x)", ylabel="f2(x)"))
	axislegend()
	f4
end
...
git-tree-sha1 = "cde15f265e916a11c877543c82fd81af6520b0a2"
