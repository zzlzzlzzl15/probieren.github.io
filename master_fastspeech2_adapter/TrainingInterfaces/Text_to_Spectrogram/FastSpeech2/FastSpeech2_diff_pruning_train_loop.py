...
class diff_pruning_fastspeeech2():
    def __init__(self):
        ...
        self.structured_diff_pruning = False
        self.alpha_init = 5
    ...
    def get_l0_norm_term(self, alpha: torch.Tensor, log_ratio: float) -> torch.Tensor:
    ...
    def get_encoder_base_modules(self, return_names: bool = False):
        ...
        return self.get_children(self.net.encoder)

    def get_encoder_base_modules_after_parameterizations(self, return_names: bool = False):
        if self._parametrized:
        ...
        return [(n,m) if return_names else m for n,m in self.net.encoder.named_modules() if check_fn(m)]
                ...
                with autocast():
                    ...
                    if step_counter <= phase_1_steps:
                        ...
                        if self._model_state == ModelState.DIFFPRUNING:
                            l0_pen = 0.
                            for module_name, base_module in self.get_encoder_base_modules_after_parameterizations(return_names=True):
    ...
    def _add_diff_parametrizations(self, *args) -> None:
        ...
        """
        for m,n in self.net.named_modules():
            print(n)
        """
        for base_module in self.get_encoder_base_modules():
            print(base_module)
    ...
    def _finetune_to_fixmask(self, pct: float) -> None:
        ...
        def _get_cutoff(values, pct):
            ...
            print(torch.topk(torch.abs(values), k, largest=True, sorted=True)[0][-1])
            return torch.topk(torch.abs(values), k, largest=True, sorted=True)[0][-1]

        if self._model_state == ModelState.DIFFPRUNING:
            diff_weights = torch.tensor([]).cuda()
            for base_module in self.get_encoder_base_modules_after_parameterizations():
                for n, par_list in list(base_module.parametrizations.items()):
                    par_list[0].eval()
                    diff_weight = (getattr(base_module, n) - par_list.original).detach().cuda()
                    diff_weights = torch.cat([diff_weights, diff_weight.flatten()]).cuda()
            ...
            for base_module in self.get_encoder_base_modules_after_parameterizations():
                for n, par_list in list(base_module.parametrizations.items()):
                    ...
                    parametrize.register_parametrization(base_module, n,
                                                         DiffWeightFixmask(pre_trained_weight, diff_mask))
            ...
            cutoff = _get_cutoff(diff_weights, pct)
            base_modules = dict(self.get_encoder_base_modules_after_parameterizations(return_names=True))
            for (n, p), p_pre,base_module in zip(list(self.net.encoder.named_parameters()), pre_trained.parameters(),self.get_encoder_base_modules_after_parameterizations()):
                n_parts = n.split(".")
    ...
    def _remove_diff_parametrizations(self) -> None:
        for module in self.get_encoder_base_modules_after_parameterizations():
