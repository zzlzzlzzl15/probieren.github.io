...
from Utility.path_to_transcript_dicts import build_path_to_transcript_dict_vctk_p251
...
def run(gpu_id, resume_checkpoint, finetune, model_dir, resume, use_wandb, wandb_resume_id):
    ...
    if model_dir is not None:
        ...
        save_dir = os.path.join(MODELS_DIR, "FastSpeech2_BitFit_vctk_p251")  # KEEP THE 'FastSpeech2_' BUT CHANGE THE MODEL ID TO SOMETHING MEANINGFUL FOR YOUR DATA
    os.makedirs(save_dir, exist_ok=True)

    datasets = list()
    datasets.append(prepare_fastspeech_corpus(transcript_dict=build_path_to_transcript_dict_vctk_p251(),
                                              corpus_dir=os.path.join(PREPROCESSING_DIR, "vctk_p251"),
